...
def render_event_card(row: pd.Series):
    ...
    source_badge = f"<span class='badge'>{row.get('source', '')}</span>" if row.get("source") else ""

    location_text = ""
    if row.get("location"):
        location_text = f"<div class='small-muted'>📍 {row.get('location')}</div>"

    description_text = ""
    if row.get("description"):
        description_text = f"<div class='small-muted'>{row.get('description')}</div>"

    st.markdown(
        f"""
        <div class="event-card">
            <div class="small-muted">{row.get('date_label', '')} · {row.get('time', '')}</div>
            <div style="font-size:1.12rem;font-weight:700;margin:0.35rem 0 0.45rem 0;">{row.get('title', '')}</div>
            {location_text}
            {description_text}
            <span class="badge">{row.get('category', 'Other')}</span>
            {parish_badge}
            {source_badge}
        </div>
        """,
